...
export function ProductCard({ product }: { product: Product }) {
  const { lang, setQuickView, addToCart, setCartOpen } = useStore();
  ...
  return (
    <div className="group flex flex-col">
      <button
        ...
        className="relative block overflow-hidden bg-secondary aspect-[4/5] text-left"
      >
        <img
          src={product.image}
          alt={name}
        ...
        {badgeLabel && (
          <span className="absolute top-3 left-3 bg-background/95 backdrop-blur text-foreground text-[10px] tracking-luxe-sm uppercase px-3 py-1.5 font-sans">
            {badgeLabel}
          </span>
        )}
        ...
        <div className="absolute inset-x-0 bottom-0 p-3 translate-y-3 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
          <span className="flex items-center justify-center gap-2 bg-background/95 backdrop-blur text-foreground text-[11px] tracking-luxe-sm uppercase py-2.5 font-sans">
      ...
      <div className="mt-4 flex items-start justify-between gap-3">
        ...
        <button
          onClick={handleAdd}
          className={cn(
            "shrink-0 h-9 w-9 rounded-full border border-border bg-background flex items-center justify-center text-foreground hover:bg-foreground hover:text-background transition-colors"
          )}
