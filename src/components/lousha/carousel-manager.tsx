          ...
          {slides.map((s, idx) => (
            ...
            >
              <img
                src={s.image}
                alt=""
                className="h-16 w-24 rounded-lg object-cover bg-secondary shrink-0"
              />
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground truncate">
                  {lang === "fr" ? s.eyebrowFr : s.eyebrowEn}
                </p>
              </div>
              <div className="flex items-center gap-1">
                ...
                <IconBtn
                  onClick={async () => {
                    ...
                    if (prev) {
                      await fetch(`/api/admin/slides/${prev.id}`, {
                        method: "PATCH",
                        headers: { "Content-Type": "application/json" },
                ...
                <IconBtn
                  onClick={async () => {
                    ...
                    if (next) {
                      await fetch(`/api/admin/slides/${next.id}`, {
                        method: "PATCH",
                        headers: { "Content-Type": "application/json" },
...
function SlideEditor({
  ...
  const handleSave = async () => {
    if (!form.image || !form.titleFr || !form.titleEn) {
      toast.error("Image + titres FR/EN requis");
      return;
    }
    setSaving(true);
    try {
      ...
      <div className="relative bg-background rounded-3xl shadow-2xl max-w-2xl w-full max-h-[92vh] overflow-y-auto scroll-elegant">
        <div className="sticky top-0 flex items-center justify-between px-6 h-16 border-b border-border bg-background z-10">
          <h2 className="font-serif text-xl">
            {slide ? t.admin.editSlide : t.admin.newSlide}
          </h2>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="Eyebrow (FR)" value={form.eyebrowFr} onChange={(v) => update("eyebrowFr", v)} />
            <Field label="Eyebrow (EN)" value={form.eyebrowEn} onChange={(v) => update("eyebrowEn", v)} />
            <Field label="Titre (FR)" value={form.titleFr} onChange={(v) => update("titleFr", v)} required />
            <Field label="Titre (EN)" value={form.titleEn} onChange={(v) => update("titleEn", v)} required />
